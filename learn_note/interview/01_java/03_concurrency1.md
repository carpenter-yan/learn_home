...
------------------------------------------------------------------------------
## 死锁的条件？
- 互斥条件 资源是排他性独占，同一时间只能一个线程占用；别的线程拿不到。比如 synchronized 锁、数据库行锁。
- 请求并保持条件 线程已经持有一部分锁/资源，同时还去申请新的资源；已经拿到的资源不会主动释放。
- 不可剥夺条件 资源不能被强行抢走。只能线程主动释放锁，别的线程没办法强制把资源抢过来。
- 循环等待条件 线程之间形成环形依赖：线程 A 拿着锁 1 等锁 2，线程 B 拿着锁 2 等锁 1，互相无限等待。

### 破坏死锁：只要打破任意一条即可避免
破互斥：尽量用无锁方案（CAS、Atomic），允许共享访问；
破请求保持：一次性申请所有需要的锁，不分批拿锁；
破不可剥夺：允许超时主动放弃锁，拿不到就释放已有资源重试；
破循环等待：所有线程统一按固定顺序获取锁（最常用）。
### 极简背诵口语版
死锁四个条件：互斥占用、持有还申请、资源抢不走、循环互相等。破坏任意一条就能杜绝死锁，业务最常用统一加锁顺序。
经典例子
线程 1：先锁 A，再锁 B
线程 2：先锁 B，再锁 A
满足四条，大概率死锁；
改成两个线程都永远先拿A、再拿B，打破循环等待，死锁消失。
------------------------------------------------------------------------------
## 死锁如何排查？
1. jstack 命令（最常用，必背）
   使用 jps -l 查到 Java 进程 PID；
   执行 jstack PID 打印完整线程栈；
   日志搜索关键字：Found one Java-level deadlock。
2. JConsole（可视化，本地调试）
   JDK自带可视化工具：
   命令行输入jconsole；
   连接当前Java进程；
   切换「线程」标签页，点击检测死锁；
   自动列出死锁线程、锁信息、堆栈。
3. JVisualVM（图形化，功能更强）
   执行 jvisualvm；
   绑定进程 → 线程 → 点击「死锁检测」；
   展示线程栈、锁持有关系，适合新手看。
4. Arthas（线上排查首选，互联网普遍用）
   线上不方便下载 jstack 文件时用阿里 Arthas：
   启动 Arthas attach 进程；
   命令：thread -b 一键阻塞线程排查，直接打印死锁线程栈、锁依赖。
### 二、拿到栈之后怎么定位代码
   找到死锁线程对应的堆栈，定位代码行数；
   核对：两个线程加锁顺序是否相反；
   确认循环等待，修改加锁顺序即可修复。
